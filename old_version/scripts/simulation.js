...
function Need(resName, duration, startTime, quantity)
{
    ...
    this.string = function(){
        ...
        " Duration=" + this.duration + " Quantity=" + this.quantity + " Priority=" + this.priority + ")";
    }
...
// Class Process
function Process(needList, arrivalTime, pID, name, priority, duration)
{ 
    ...
    this.priority = (priority !== undefined) ? priority : "None";
	print("------- " + name + " has priority of " + this.priority);
	this.duration = (duration !== undefined) ? duration : "Error";
	print("------- " + name + " has duration of " + this.duration);
    //Metric variables:
    ...
    //Metric functions:
	this.getArrivalTime = function(){ return this.arrivalTime; }
    this.getWaitDuration = function(){ return this.waitDuration; }
    this.getFirstProgressTime = function(){ return (this.firstProgressTime != -1) ? this.firstProgressTime : "N/A"; }
    this.getTurnAroundTime = function() { return (this.finishTime != -1 ) ? (this.getWaitDuration() + this.duration) : "N/A"; }
    ...
    this.getTotalRunDuration = function() { 
        ...
        for(var ii in this.needList){ 
			if (this.needList[ii].getEndTime()  > retVal) { 
				retVal = this.needList[ii].getEndTime(); 
			}
		}
    return retVal;
    }
...
function Resource(name, initialClock)
{
    ...
    this.addRequests = function(requestList)
    {
        ...
        for(var ii in requestList){ 
            ...
            for (var jj in this.waitingQueue){ 
    ...
    this.currentAllocations = function()
    {
        ...
        if (this.waitingQueue.length > 0){
			...
			document.getElementById("sim-info").innerHTML = "<pre>	  Arrival	Duration	Priority	  WaitingTime		TurnaroundTime</pre></p></br>";
				for(var s in procArray){
					content += ("<p><pre>Process " + procArray[s].name + ":  "+ procArray[s].arrivalTime + "		" + procArray[s].duration + "		" + procArray[s].priority+
...
function Simulator(processList, resourceList, terminatedProcList, simulatorClock, lastEventString)
{
	...
	for (var ii in this.processList){
		if (this.processList[ii].arrivalTime > this.simulatorClock){
			...
			print("@@@@@ Moving procss " + ii +" from processList to waitingProcList ");
    ...
    { // Go through each process, ask it it's current requests. Accumulate them by resource.
        var requestDict = {}; 
			...
			for (var ii in this.processList){ 
				...
				print("------- " + proc.name + "'s priority = " + proc.priority);
    ...
    this.simNextEvent = function()
    {
        ...
        for (var ii in needDict) {
            ...
            if (res !== undefined){ 
				res.addRequests(needDict[ii]); 
			}else{ 
				print("ERROR! cannot find requested resource \""+ii+"\" in this simulation."); 
			}
        }
        //4.) Get the current allocations from each resource. Accumulate them in lists by process.
