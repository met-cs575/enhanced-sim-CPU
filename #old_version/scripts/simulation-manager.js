    ...
    return {
        ...
        step: function() {
            ...
            if(nextUpdate == 0) {
				...
				// Update the final information and statistic result 
				document.getElementById("result-info").innerHTML = "<pre>Avg. Waiting Time			Avg. Turnaround Time</pre></p></br>";
				for(var s in procArray){
					sum_w += procArray[s].getWaitDuration();
					sum_tt += procArray[s].getTurnAroundTime();
					info += ("<pre>Process " + procArray[s].name + ":  "
									+ procArray[s].arrivalTime + "		" + procArray[s].duration + "		" 
									+ procArray[s].priority+"		  " + procArray[s].getWaitDuration()+ "   			"
									+ (procArray[s].duration+ procArray[s].getWaitDuration()) + "</pre></br>");
				}
				content += ("<pre>" + parseFloat(sum_w/procArray.length).toFixed(2) + "					" 
							+ parseFloat(sum_tt/procArray.length).toFixed(2) +"</pre></br>"); 
